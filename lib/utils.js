...
function replaceVueTemplate(_a) {
    var content = _a.content, file = _a.file, getKey = _a.getKey, replaceSuccess = _a.replaceSuccess, replaceFail = _a.replaceFail;
    return content.replace(/<template(.|\n|\r)*template>/gim, function (match) {
        var originMatch = match;
        return match.replace(/((\w+-){0,}\w+=['"]|>|'|")([^'"<>]*[\u4e00-\u9fa5]+[^'"<>]*)(['"<])/gim, function (_, prev, __, match, after, offset) {
            // 针对一些资源中含有中文名时，不做替换
            if (prev.match(/src=['"]/))
                return _;
            // 针对使用中文键的变量不做处理
            if (i18nFile_1.VueI18nInstance.getConfig().useChineseKey) {
                var chineseTransformKey = originMatch.slice(offset - 3, offset + match.length + 3);
            ...
            match = match.trim();
            var result = '';
            var replaceStringFail = false; // 替换失败
            var currentKey;
            if (match.match(/{{[^{}]+}}/)) {
                // 包含变量的中文字符串
                var matchIndex_1 = 0;
                ...
                if (prev.match(/^(\w+-){0,}\w+='$/)) {
                    ...
                    replaceFail && replaceFail({ currentKey: currentKey, match: match });
                    replaceStringFail = true;
                }
            }
            replaceStringFail === false && replaceSuccess && replaceSuccess({ currentKey: currentKey, match: match });
            return result;
        });
    });
}
...
function replaceVueScript(_a) {
    ...
    return content.replace(/<script(.|\n|\r)*script>/gim, function (match) {
        ...
        match = match.replace(
        // (?!\1) 指 非 ['"`]
        /(['"`])(((?!\1).)*[\u4e00-\u9fa5]+((?!\1).)*)\1/gim, function (_, prev, match, __, ___, offset) {
            // 针对使用中文键的变量不做处理
            if (i18nFile_1.VueI18nInstance.getConfig().useChineseKey) {
                var chineseTransformKey = originMatch.slice(offset - 3, offset + match.length + 3);
                if (/^\$t\(.+\)/.test(chineseTransformKey)) {
            ...
            replaceSuccess && replaceSuccess({ currentKey: currentKey, match: match });
            return result;
        });
        // 换回注释 和 require
...
function replaceJavaScript(_a) {
    ...
    var originMatch = content;
    content = content.replace(/(['"`])(((?!\1).)*[\u4e00-\u9fa5]+((?!\1).)*)\1/gim, function (_, prev, match, __, ___, offset) {
        // 针对使用中文键的变量不做处理
        if (i18nFile_1.VueI18nInstance.getConfig().useChineseKey) {
            var chineseTransformKey = originMatch.slice(offset - 3, offset + match.length + 3);
            if (/^\.t\(.+\)/.test(chineseTransformKey)) {
